...
fun SeerrSignInScreen(
    viewModel: SeerrSignInViewModel = koinViewModel(),
    onNavigateBack: () -> Unit
) {
    ...
    LaunchedEffect(viewModel.sideEffect) {
        viewModel.sideEffect.collect { effect ->
            when (effect) {
                SeerrSignInSideEffect.LoginSuccess -> onNavigateBack()
            }
        }
    }
    ...
    SeerSignInContent(
        ...
        onServerUrlChanged = { serverUrl ->
            viewModel.onEvent(SeerrSignInEvent.ServerUrlChanged(serverUrl))
        },
        onUsernameChanged = { username ->
            viewModel.onEvent(SeerrSignInEvent.UsernameChanged(username))
        },
        onPasswordChanged = { password ->
            viewModel.onEvent(SeerrSignInEvent.PasswordChanged(password))
        },
        onAuthChanged = { authType ->
            viewModel.onEvent(SeerrSignInEvent.AuthTypeChanged(authType))
        },
        onSubmitClick = {
            viewModel.onEvent(SeerrSignInEvent.Submit)
        }
    )
}
...
private fun SeerSignInContent(
    state: SeerrSignInState,
    onNavigateBack: () -> Unit,
    onServerUrlChanged: (serverUrl: String) -> Unit,
    onUsernameChanged: (username: String) -> Unit,
    onPasswordChanged: (password: String) -> Unit,
    onAuthChanged: (authType: SeerrAuthType) -> Unit,
    onSubmitClick: () -> Unit
) {
    ...
    Scaffold(
        ...
        Column(
            ...
            TabRow(selectedTabIndex = state.authType.ordinal) {
                SeerrAuthType.entries.forEachIndexed { index, type ->
                    Tab(
                        ...
                        text = {
                            Text(
                                when (type) {
                                    SeerrAuthType.LOCAL -> stringResource(R.string.seerr_auth_local)
                                    SeerrAuthType.JELLYFIN -> stringResource(R.string.seerr_auth_jellyfin)
                                }
                            )
                        }
                    )
            ...
            OutlinedTextField(
                ...
                label = {
                    Text(
                        when (state.authType) {
                            SeerrAuthType.LOCAL -> stringResource(R.string.seerr_email_label)
                            SeerrAuthType.JELLYFIN -> stringResource(R.string.seerr_username_label)
                        }
                    )
                },
                singleLine = true,

...
class SeerrRequestViewModel(
    ...
    private val getMediaInfoUseCase: GetSeerrMediaInfoUseCase,
    private val createRequestUseCase: CreateSeerrRequestUseCase,
    ...
    private fun submit() {
        ...
        viewModelScope.launch {
            ...
            createRequestUseCase(state.mediaType, state.mediaId, seasons)
                .onSuccess {
                    state = state.copy(isSubmitting = false, ableToSubmit = isAbleToSubmit())
//                    statusProvider.invalidate(state.mediaType, state.mediaId)
                    statusProvider.statusFor(state.mediaType, state.mediaId)
                    _sideEffect.send(SeerrRequestSideEffect.Dismiss)
                }
                .onError {
                    state = state.copy(isSubmitting = false, ableToSubmit = isAbleToSubmit())
